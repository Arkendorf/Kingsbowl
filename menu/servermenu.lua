function servermenu_load()
  team = {{name = "Team 1", r = 250, g = 0, b = 0, playerNum = 0}, {name = "Team 2", r = 0, g = 0, b = 255, playerNum = 0}}
...
function servermenu_update(dt)
  if proceed == false then
    ...
    if team[1].name ~= sentTeam1.name or team[1].r + team[1].g + team[1].b ~= sentTeam1.r + sentTeam1.g + sentTeam1.b or team[2].name ~= sentTeam2.name or team[2].r + team[2].g + team[2].b ~= sentTeam2.r + sentTeam2.g + sentTeam2.b or newPlayer == true then
      server:send(bin:pack({"teams", team[1].name, team[1].r, team[1].g, team[1].b, team[2].name, team[2].r, team[2].g, team[2].b}))
      sentTeam1.name, sentTeam1.r, sentTeam1.g, sentTeam1.b = team[1].name, team[1].r, team[1].g, team[1].b
      sentTeam2.name, sentTeam2.r, sentTeam2.g, sentTeam2.b = team[2].name, team[2].r, team[2].g, team[2].b
    end
    ...
    for p = 1, #players do
      if players[p].team == 1 then
        team[1].playerNum = team[1].playerNum + 1
      else
        team[2].playerNum = team[2].playerNum + 1
      end
    end
...
function servermenu_draw()
  if proceed == false then
    ...
    love.graphics.draw(sliderImg, knob, 12 + math.floor(team[1].r / 5.10), 22)
    love.graphics.draw(sliderImg, knob, 12 + math.floor(team[1].g / 5.10), 30)
    love.graphics.draw(sliderImg, knob, 12 + math.floor(team[1].b / 5.10), 38)
    ...
    love.graphics.draw(sliderImg, knob, 187 + math.floor(team[2].r / 5.10), 22)
    love.graphics.draw(sliderImg, knob, 187 + math.floor(team[2].g / 5.10), 30)
    love.graphics.draw(sliderImg, knob, 187 + math.floor(team[2].b / 5.10), 38)
...
function servermenu_mousepressed(x, y, button)
  if proceed == false then
    ...
    if button == 1 and start == false then
      ...
      elseif x >= 87 + team[1].g / 5.10 and x <= 87 + team[1].g / 5.10 + 2 and y >= 80 and y <= 80 + 6 then
        slider = {type = "g1", xPos = x, yPos = y, old = team[1].g}
      elseif x >= 87 + team[1].b / 5.10 and x <= 87 + team[1].b / 5.10 + 2 and y >= 88 and y <= 88 + 6 then
        slider = {type = "b1", xPos = x, yPos = y, old = team[1].b}
      elseif x >= 262 + team[2].r / 5.10 and x <= 262 + team[2].r / 5.10 + 2 and y >= 72 and y <= 72 + 6 then
        slider = {type = "r2", xPos = x, yPos = y, old = team[2].r}
      elseif x >= 262 + team[2].g / 5.10 and x <= 262 + team[2].g / 5.10 + 2 and y >= 80 and y <= 80 + 6 then
        slider = {type = "g2", xPos = x, yPos = y, old = team[2].g}
      elseif x >= 262 + team[2].b / 5.10 and x <= 262 + team[2].b / 5.10 + 2 and y >= 88 and y <= 88 + 6 then
        slider = {type = "b2", xPos = x, yPos = y, old = team[2].b}
...
function servermenu_textinput(text)
  if proceed == false then
    ...
    if textBox == "team1" and getPixelWidth(team[1].name .. text) < 67 then
      team[1].name = team[1].name .. text
    elseif textBox == "team2" and getPixelWidth(team[2].name .. text) < 67 then
      team[2].name = team[2].name .. text
    end
  end
end
...
function servermenu_keypressed(key)
  if proceed == false then
    ...
    if key == "backspace" then
      if textBox == "team1" then
        team[1].name = string.sub(team[1].name, 1, -2)
      elseif textBox == "team2" then
        team[2].name = string.sub(team[2].name, 1, -2)
      end
    end
  end
end
...
function updateSlider(slider)
  if love.mouse.isDown(1) == true then
    if slider.type == "r1" then
      team[1].r = range((x - slider.xPos) * 5.10 + slider.old, 0, 255)
    elseif slider.type == "g1" then
      team[1].g = range((x - slider.xPos) * 5.10 + slider.old, 0, 255)
    elseif slider.type == "b1" then
      team[1].b = range((x - slider.xPos) * 5.10 + slider.old, 0, 255)
    elseif slider.type == "r2" then
      team[2].r = range((x - slider.xPos) * 5.10 + slider.old, 0, 255)
    elseif slider.type == "g2" then
      team[2].g = range((x - slider.xPos) * 5.10 + slider.old, 0, 255)
    elseif slider.type == "b2" then
      team[2].b = range((x - slider.xPos) * 5.10 + slider.old, 0, 255)
    end
  else
    slider.type = ""
  end
